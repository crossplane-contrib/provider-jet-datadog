...
// JSON is the Schema for the JSONs API. <no value>
...
type JSON struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.monitor) || has(self.initProvider.monitor)",message="monitor is a required parameter"
